...
;(function (global, factory) {
    ...
    function valid__isValid(m) {
        if (m._isValid == null) {
            ...
            m._isValid = !Number.isNaN(m._d.getTime()) &&
                flags.overflow < 0 &&
                !flags.empty &&
                !flags.invalidMonth &&
    ...
    function normalizeLocale(key) {
        return key ? key.toLowerCase().replace(/\s+/g, ' ') : key;
    }
    ...
    function localeMonthsParse (monthName, format, strict) {
        ...
        for (i = 0; i < 12; i++) {
            ...
            if (strict && !this._longMonthsParse[i]) {
                this._longMonthsParse[i] = new RegExp('^' + this.months(mom, '').replace(/\s+/g, ' ') + '$', 'i');
                this._shortMonthsParse[i] = new RegExp('^' + this.monthsShort(mom, '').replace(/\s+/g, ' ') + '$', 'i');
            }
            if (!strict && !this._monthsParse[i]) {
                regex = '^' + this.months(mom, '') + '|^' + this.monthsShort(mom, '');
                this._monthsParse[i] = new RegExp(regex.replace(/\s+/g, ' '), 'i');
            }
            // test the regex
    ...
    function parseIso (inp, sign) {
        ...
        var res = inp && parseFloat(inp.replace(/\s+/g, ' '));
    ...
    function localeWeekdaysParse (weekdayName, format, strict) {
        ...
        for (i = 0; i < 7; i++) {
            ...
            if (strict && !this._fullWeekdaysParse[i]) {
                this._fullWeekdaysParse[i] = new RegExp('^' + this.weekdays(mom, '').replace(/\s+/g, ' ') + '$', 'i');
                this._shortWeekdaysParse[i] = new RegExp('^' + this.weekdaysShort(mom, '').replace(/\s+/g, ' ') + '$', 'i');
                this._minWeekdaysParse[i] = new RegExp('^' + this.weekdaysMin(mom, '').replace(/\s+/g, ' ') + '$', 'i');
            }
            if (!this._weekdaysParse[i]) {
                regex = '^' + this.weekdays(mom, '') + '|^' + this.weekdaysShort(mom, '') + '|^' + this.weekdaysMin(mom, '');
                this._weekdaysParse[i] = new RegExp(regex.replace(/\s+/g, ' '), 'i');
            }
            // test the regex
...
module.exports = function(Chart) {
	...
	Chart.controllers.bubble = Chart.DatasetController.extend({
		...
		updateElement: function(point, index, reset) {
			...
			point._model.skip = point.custom && point.custom.skip ? point.custom.skip : (Number.isNaN(point._model.x) || Number.isNaN(point._model.y));
	...
	Chart.controllers.doughnut = Chart.controllers.pie = Chart.DatasetController.extend({
		...
		calculateTotal: function() {
			...
			helpers.each(meta.data, function(element, index) {
				...
				if (!Number.isNaN(value) && !element.hidden) {
	...
	Chart.controllers.polarArea = Chart.DatasetController.extend({
		...
		updateElement: function(arc, index, reset) {
			...
			for (var i = 0; i < index; ++i) {
				if (!Number.isNaN(this.getDataset().data[i]) && !meta.data[i].hidden) {
					++visibleCount;
				}
			}
		...
		countVisibleElements: function() {
			...
			helpers.each(meta.data, function(element, index) {
				if (!Number.isNaN(dataset.data[index]) && !element.hidden) {
					count++;
				}
			});
...
module.exports = function(Chart) {
	...
	helpers.extend(Chart.Controller.prototype, {
		...
		resize: function resize(silent) {
			...
			var newHeight = (this.options.maintainAspectRatio && Number.isNaN(this.chart.aspectRatio) === false && isFinite(this.chart.aspectRatio) && this.chart.aspectRatio !== 0) ? newWidth / this.chart.aspectRatio : helpers.getMaximumHeight(this.chart.canvas);
    ...
    transition: function(ease) {
      ...
      helpers.each(this._model, function(value, key) {
        ...
        // Init if doesn't exist
          ...
          if (typeof value === 'number' && !Number.isNaN(this._view[key])) {
        ...
        // Number transitions
          ...
          var startVal = this._start[key] !== undefined && Number.isNaN(this._start[key]) === false ? this._start[key] : 0;
...
module.exports = function() {
	...
	var Chart = function(context, config) {
		...
		if (Number.isNaN(this.aspectRatio) || isFinite(this.aspectRatio) === false) {
...
module.exports = function(Chart) {
	...
	Chart.elements.Point = Chart.Element.extend({
		...
		draw: function() {
			...
			if (!Number.isNaN(vm.radius) && vm.radius > 0) {
				...
				helpers.each(this.chart.data.datasets, function(dataset, datasetIndex) {
					...
					if (this.chart.isDatasetVisible(datasetIndex) && (this.isHorizontal() ? meta.xAxisID === this.id : meta.yAxisID === this.id)) {
						helpers.each(dataset.data, function(rawValue, index) {
							...
							if (Number.isNaN(value) || meta.data[index].hidden) {
								return;
							}
