...
import static rc.soop.action.ActionB.insertConvenzioneROMA;
import static rc.soop.action.ActionB.insertDocumentUserConvenzioni;
import static rc.soop.action.ActionB.isPresenzaDocumento;
import static rc.soop.action.ActionB.listaDocRichiesti;
import static rc.soop.action.ActionB.preparefileforupload;
import static rc.soop.action.ActionB.trackingAction;
import static rc.soop.action.ActionB.verificaPresenzaConvenzioni;
...
import static rc.soop.action.Pdf_new.extractSignatureInformation_PDF;
import static rc.soop.action.Pdf_new.verificaQR;
import static rc.soop.action.Pdf_new.verificaQRNOPDFA;
...
import static rc.soop.util.Utility.checkExtension;
import static rc.soop.util.Utility.checkFile;
                        ...
                        if (fileName != null) {
                            if (!tipodoc.equals("")) {
                                if (fileName.lastIndexOf(".") > 0) {
                                    File pathdir = createFile(pathtemp + el.toString("yyyyMMdd"));
                                    pathdir.mkdirs();
                                    ...
                                    if (estensione.equalsIgnoreCase(".pdf") || estensione.equalsIgnoreCase(".p7m")) {
                                        ...
                                        nomefile = createFile(pathdir + separator + name);
                                        try {
                                            item.write(nomefile.toPath());
                                        } catch (Exception ex) {
        ...
        if (nomefile != null) {
            if (getExtension(nomefile.getName()).endsWith("p7m")) {
                ...
                if (Constant.test) {
                    ...
                    switch (tipodoc) {
                        case "DONLA", "DONLB", "CONV", "MOD1", "MOD2", "DONLC", "DONLF" -> {
                            //DOCUMENTI CHE DEVONO ESSERE FIRMATI
                            SignedDoc dc = extractSignatureInformation_PDF(readFileToByteArray(nomefile), createFile(nomefile.getPath() + "_tempcheck.pdf"));
                            if (dc.isValido()) {
                                String cfuser = dbb.getCF_user(username);
                                if (!dc.getCodicefiscale().toUpperCase().contains(cfuser.toUpperCase())) {
                                    msg = "RA2"; //
            ...
            if (getExtension(nomefile.getName()).endsWith("p7m")) {
                ...
                if (Constant.test) { //RIMUOVERE
                    ...
                    if (tipodoc.equals("DONLA")
                            ...
                            || tipodoc.equals("MOD1")
                            || tipodoc.equals("MOD2")
                            || tipodoc.equals("MOD3")) { //DOCUMENTI CHE DEVONO ESSERE FIRMATI
                        SignedDoc dc = extractSignatureInformation_PDF(readFileToByteArray(nomefile),
                                createFile(nomefile.getPath() + "_tempcheck.pdf"));
                        if (dc.isValido()) {
