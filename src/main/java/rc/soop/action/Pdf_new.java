...
import static rc.soop.util.Utility.createFile;
...
public class Pdf_new {
    ...
    private static File allegatoA(String username, DateTime dataconsegna, boolean flatten) {
        try {
            ...
            File pdfOut = createFile(pathtemp + username + dataconsegna.toString("ddMMyyyyHHmmSSS") + ".A.pdf");
            try (InputStream is = new ByteArrayInputStream(decodeBase64(contentb64));
                ...
                try {
                    Optional<String> comunecciaaOpt = listavalori.stream()
                            .filter(re -> re.getCampo().equals("comunecciaa"))
                            .map(re -> re.getValore().toUpperCase())
                            .findAny();

                    Optional<String> proccciaaOpt = listavalori.stream()
                            .filter(re -> re.getCampo().equals("proccciaa"))
                            .map(re -> re.getValore().toUpperCase())
                            .findAny();

                    Optional<String> regccciaaOpt = listavalori.stream()
                            .filter(re -> re.getCampo().equals("regccciaa"))
                            .map(re -> re.getValore().toUpperCase())
                            .findAny();

                    if (comunecciaaOpt.isPresent() && proccciaaOpt.isPresent() && regccciaaOpt.isPresent()) {
                        String comunecciaa = comunecciaaOpt.get();
                        String proccciaa = proccciaaOpt.get();
                        String regccciaa = regccciaaOpt.get();

                        if (!comunecciaa.isEmpty() && !proccciaa.isEmpty() && !regccciaa.isEmpty()) {
                            String cciaa = comunecciaa + " - " + proccciaa + " - " + regccciaa;
                            setFieldsValue(form, fields, "cciaa", cciaa);
                        }
                    }
                } catch (Exception e) {
                    trackingAction("error optional pdfnew class line149", estraiEccezione(e));
                }

                fields.forEach((K, V) -> {

                    UserValoriReg va = listavalori.stream().filter(re -> re.getCampo().equals(K)).findAny()
                            .orElse(null);
                    if (va != null) {
                ...
                BarcodeQRCode barcode = new BarcodeQRCode(
                        username + " / ALLEGATO A / " + dataconsegna.toString("ddMMyyyyHHmmSSS"));
    ...
    private static File allegatoB(String username, DateTime dataconsegna, boolean flatten) {
        try {
            ...
            dbb.closeDB();
            createDir(pathtemp);
            File pdfOut = createFile(pathtemp + username + dataconsegna.toString("ddMMyyyyHHmmSSS") + ".B.pdf");
            ...
            try (InputStream is = new ByteArrayInputStream(decodeBase64(contentb64));
                ...
                try {
                    ...
                    trackingAction("error optional (pdfnew class)", estraiEccezione(e));
                }
                ...
                BarcodeQRCode barcode = new BarcodeQRCode(
                        username + " / ALLEGATO B / " + dataconsegna.toString("ddMMyyyyHHmmSSS"));
                printbarcode(barcode, pdfDoc);
            }
            if (checkPDF(pdfOut)) {
                return pdfOut;
    ...
    private static File allegatoB1(String username, String iddocente, DateTime dataconsegna, boolean flatten) {
        try {
            ...
            dbb.closeDB();
            createDir(pathtemp);
            File pdfOut = createFile(pathtemp + username + dataconsegna.toString("ddMMyyyyHHmmSSS") + ".B1.pdf");
            try (InputStream is = new ByteArrayInputStream(decodeBase64(contentb64));
                    PdfReader reader = new PdfReader(is);
                    PdfWriter writer = new PdfWriter(pdfOut);
                    PdfDocument pdfDoc = new PdfDocument(reader, writer);) {
                ...
                BarcodeQRCode barcode = new BarcodeQRCode(
                        username + " / ALLEGATO B1 / " + dataconsegna.toString("ddMMyyyyHHmmSSS"));
                printbarcode(barcode, pdfDoc);
    ...
    private static File allegatoC(String username, DateTime dataconsegna, boolean flatten) {
        try {
            ...
            createDir(pathtemp);
            File pdfOut = createFile(pathtemp + username + dataconsegna.toString("ddMMyyyyHHmmSSS") + ".C.pdf");
            try (InputStream is = new ByteArrayInputStream(decodeBase64(contentb64));
                    PdfReader reader = new PdfReader(is);
                    PdfWriter writer = new PdfWriter(pdfOut);
                    PdfDocument pdfDoc = new PdfDocument(reader, writer)) {
                ...
                setFieldsValue(form, fields, "nomecognome",
                        listavalori.stream().filter(va1 -> (va1.getCampo().equals("nome"))).findAny().orElse(null)
                                .getValore() + " "
                                + listavalori.stream().filter(va1 -> (va1.getCampo().equals("cognome"))).findAny()
                                        .orElse(null).getValore());

                if (flatten) {
                    form.flattenFields();
                }
                BarcodeQRCode barcode = new BarcodeQRCode(
                        username + " / ALLEGATO C / " + dataconsegna.toString("ddMMyyyyHHmmSSS"));
                printbarcode(barcode, pdfDoc);
            }
    ...
    private static File allegatoC_1(String username, DateTime dataconsegna) {
        try {
            ...
            try (InputStream is = new ByteArrayInputStream(decodeBase64(contentb64));
                    ...
                    PdfDocument pdfDoc = new PdfDocument(reader, writer)) {
                BarcodeQRCode barcode = new BarcodeQRCode(
                        username + " / ALLEGATO 1 / " + dataconsegna.toString("ddMMyyyyHHmmSSS"));
                printbarcode(barcode, pdfDoc);
            }
            if (checkPDF(pdfOut)) {
                return pdfOut;
    ...
    private static File allegatoC_2(String username, DateTime dataconsegna, boolean flatten) {
        try {
            ...
            createDir(pathtemp);
            File pdfOut = createFile(pathtemp + username + dataconsegna.toString("ddMMyyyyHHmmSSS") + ".C2.pdf");
            try (InputStream is = new ByteArrayInputStream(decodeBase64(contentb64));
                    PdfReader reader = new PdfReader(is);
                    PdfWriter writer = new PdfWriter(pdfOut);
                    PdfDocument pdfDoc = new PdfDocument(reader, writer)) {
                            ...
                            try {
                                ...
                                if (K.equals(name)) {
                                    ...
                                    if (checkbox) {
                                        ...
                                        setFieldsValue(form, fields, K,
                                                field.get(allegato_c2).toString().toUpperCase());
                                    }
                                }
                            } catch (Exception e) {
                                trackingAction("ERROR SYSTEM", estraiEccezione(e));
                ...
                if (allegato_c2.getSoggetto1_nome().equals("") || allegato_c2.getSoggetto1_cognome().equals("")) {
                    ...
                    setFieldsValue(form, fields, "soggetto1_nomecognome", allegato_c2.getSoggetto1_nome().toUpperCase()
                            + " " + allegato_c2.getSoggetto1_cognome().toUpperCase());
                }

                if (allegato_c2.getSoggetto2_nome().equals("") || allegato_c2.getSoggetto2_cognome().equals("")) {
                } else {
                    setFieldsValue(form, fields, "soggetto2_nomecognome", allegato_c2.getSoggetto2_nome().toUpperCase()
                            + " " + allegato_c2.getSoggetto2_cognome().toUpperCase());
                }

                if (allegato_c2.getSoggetto1_luogonascita().equals("")
                        || allegato_c2.getSoggetto1_datanascita().equals("")) {
                } else {
                    setFieldsValue(form, fields, "soggetto1_luogodata",
                            allegato_c2.getSoggetto1_luogonascita().toUpperCase() + " - "
                                    + allegato_c2.getSoggetto1_datanascita());
                }

                if (allegato_c2.getSoggetto2_luogonascita().equals("")
                        || allegato_c2.getSoggetto2_datanascita().equals("")) {
                } else {
                    setFieldsValue(form, fields, "soggetto2_luogodata",
                            allegato_c2.getSoggetto2_luogonascita().toUpperCase() + " - "
                                    + allegato_c2.getSoggetto2_datanascita());
                }

                if (allegato_c2.getSoggetto1_indirizzoresidenza().equals("")
                        || allegato_c2.getSoggetto1_cittaresidenza().equals("")) {
                } else {
                    setFieldsValue(form, fields, "soggetto1_residenza",
                            allegato_c2.getSoggetto1_indirizzoresidenza().toUpperCase() + " "
                                    + allegato_c2.getSoggetto1_cittaresidenza().toUpperCase());
                }
                if (allegato_c2.getSoggetto2_indirizzoresidenza().equals("")
                        || allegato_c2.getSoggetto2_cittaresidenza().equals("")) {
                } else {
                    setFieldsValue(form, fields, "soggetto2_residenza",
                            allegato_c2.getSoggetto2_indirizzoresidenza().toUpperCase() + " "
                                    + allegato_c2.getSoggetto2_cittaresidenza().toUpperCase());
                }
                ...
                BarcodeQRCode barcode = new BarcodeQRCode(
                        username + " / ALLEGATO 2 / " + dataconsegna.toString("ddMMyyyyHHmmSSS"));
                printbarcode(barcode, pdfDoc);
            }
            if (checkPDF(pdfOut)) {
                return pdfOut;
    ...
    public static File nullaosta(String username, String protocollo, String nomesa, DateTime dataconsegna) {
        try {
            ...
            dbb.closeDB();
            createDir(pathtemp);
            File pdfOut = createFile(pathtemp + username + dataconsegna.toString("ddMMyyyyHHmmSSS") + ".NO.pdf");
            ...
            try (PdfReader reader = new PdfReader(is);
                ...
                // fields.get("strategie").setValue("PROGETTO YES I START UP");
                // fields.get("soggetti").setValue("NEET IN FORMAZIONE");
                // fields.get("ambito").setValue("NAZIONALE");
                // fields.get("output").setValue("PERCORSO FORMATIVO (FORMAZIONE NEET)");
                // fields.get("finalita").setValue("REALIZZAZIONE DA PARTE DEL S.A. DELLE
                // ATTIVITÀ PREVISTE DALL'AVVISO PUBBLICO VOLTE ALLA FORMAZIONE "
                // + "E ALL'ACCOMPAGNAMENTO ALL'AUTOIMPIEGO E ALL'AUTOIMPRENDITORIALITÀ DEI
                // GIOVANI NEET SU TUTTO IL TERRITORIO NAZIONALE.");
                ...
                // DINAMICI
                // fields.get("protocollo").setValue(protocollo);
                // fields.get("enm").setValue(nomesa);
                // fields.get("data").setValue(dataconsegna.toString("dd/MM/yyyy"));
                form.flattenFields();

                BarcodeQRCode barcode = new BarcodeQRCode(
                        username + " / NULLA OSTA / " + dataconsegna.toString("ddMMyyyyHHmmSSS"));
    ...
    //// PdfDocument pdfDoc = new PdfDocument(new PdfReader(f1), new
    //// PdfWriter(pdfOut));
    ...
    //// fields.get("otp").setValue("FIRMATO TRAMITE OTP IN DATA " + new
    ...
    private static File convertPDFA(File pdf_ing, String nomepdf) {
        ...
        try {
            ...
            File pdfOutA = createFile(replace(pdf_ing.getPath(), ".pdf", "_pdfA.pdf"));
    ...
    //// if (out2 != null) {
    //// File out3 = convertPDFA(out2, "Allegato A",
    //// Base64.decodeBase64(ActionB.getPath("icc")));
    //// if (out3 != null) {
    //// return out3;
    //// }
    //// }
    ...
    //// return null;
    //// }
    //// public static File allegatoB_firmaotp(String username) {
    //// File out1 = allegatoB(username, new DateTime(), false);
    //// if (out1 != null) {
    //// File out2 = sign_otp(out1, username);
    //// if (out2 != null) {
    //// File out3 = convertPDFA(out2, "Allegato B",
    //// Base64.decodeBase64(ActionB.getPath("icc")));
    //// if (out3 != null) {
    //// return out3;
    //// }
        ...
        try {
            ...
            if (!allCerts.isEmpty()) {
                ...
                try (InputStream in = new ByteArrayInputStream(x509h.getEncoded())) {
                    ...
                    if (doc.isValido()) {
                        // String cf = substringBefore(substringAfter(principal.getName(),
                        // "SERIALNUMBER="), ", GIVENNAME");
                        ...
                        if (new DateTime(cert.getNotAfter().getTime()).isBeforeNow()) {
                            doc.setErrore("CERTIFICATO SCADUTO IN DATA "
                                    + new DateTime(cert.getNotAfter().getTime()).toString("dd/MM/yyyy HH:mm:ss"));
                        }
                    }
                }
            } else {
    ...
    public static String verificaPDFA(String codicedoc, String username, byte[] content) {
        ...
        //// setProperty("sun.java2d.cmm", "sun.java2d.cmm.kcms.KcmsServiceProvider");
        //// if (content != null) {
        //// try ( InputStream is1 = new ByteArrayInputStream(content); PDDocument doc =
        // load(is1)) {
        //// PDDocumentInformation info = doc.getDocumentInformation();
        ////
                    ...
                    try (PDDocument doc = loadPDF(content)) {
                        ...
                        if (qr.contains(username.toUpperCase())) {
                            switch (codicedoc) {
                                ...
                                default -> out = "ERRORE - DOCUMENTO NON CORRISPONDE A QUANTO RICHIESTO";
                            }
                        } else {
    ...
    public static boolean checkPDF(File pdffile) {
        File safeFile = createFile(pdffile.getPath());
    ...
    public static void setFieldsValue(PdfAcroForm form, Map<String, PdfFormField> fields_list, String field_name,
            String field_value) {
        try {
