...
//            }
    ...
    public static ArrayList<Docuserbandi> listaDocUserBando_nofilemcn(String cod, String username) {
        Db_Bando dbb = new Db_Bando();
        ArrayList<Docuserbandi> liout = dbb.listaDocUserBando_nofilemcn(cod, username);
        dbb.closeDB();
        return liout;
    }
    ...
    public static boolean insAllegatoA(
        ...
        boolean ctrl = db.insAllegatoA(
                ...
                //AULA 2
                indirizzo2, citta2,
                provincia2, regioneaula2, titolo3, estremi2, accreditamento2,
    ...
    public static String listaDocUserBandoDocenti(String username, String cod, String iddocente) {
        Db_Bando db = new Db_Bando();
        String desc = db.listaDocUserBandoDocenti(username, cod, iddocente);
        db.closeDB();
        return desc;
    }
    ...
    public static String preparefileforupload(File file) {
        try {
            ...
            // Use the sanitized and validated file
            File safeFile = createFile(file.getPath());
            String mimeType = new Tika().detect(safeFile);
            String content = Base64.getEncoder().encodeToString(readFileToByteArray(safeFile));
            return safeFile.getName() + "###" + mimeType + "###" + content;
        } catch (Exception ex) {
    ...
    public static File creaZip_Convenzione(String username, String protocollo, String ragsoc) {
        try {
            ...
            for (Docuserconvenzioni doc : aldocumenti) {
                if (!doc.getPath().equals("-")) {
                    if (doc.getCodicedoc().contains("CONV")) {
                        ff.add(preparefilefordownload((doc.getPath())));
                    }
                }
            }
            if (no != null) {
            ...
            File zipout = createFile(pathtemp + name); // Use the constructor to handle file separator
